	...
	def forward(self, x):
		...
		if self.use_spatial:
			...
			if not self.use_channel:
				...
				spatial_out = torch.sigmoid(W_ys.expand_as(x))
				x = spatial_out * x
		...
		if self.use_channel:
			...
			channel_out = torch.sigmoid(W_yc)
...
# 		self.gx_spatial = nn.Sequential(
# 			nn.Conv2d(in_channels=self.in_channel, out_channels=512,
# 						kernel_size=1, stride=1, padding=0, bias=False),
# 			nn.ReLU(),
# 			nn.Conv2d(in_channels=512, out_channels=8,
# 						kernel_size=1, stride=1, padding=0, bias=False),
# 		)
