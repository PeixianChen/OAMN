...
from reid.datasets.domain_adaptation_market import DA
...
def get_data(data_dir, source, target, height, width, batch_size, num_instances=2, re=0, workers=8):
    ...
    source_train_loader = DataLoader(
        ...
        sampler=IdentitySampler(dataset.source_train, num_instances),
        pin_memory=True, drop_last=True)
    # partial-iLIDS
    query_loader_2 = DataLoader(
        Preprocessor_occluded(dataset.query_2,
                     root=osp.join(dataset.target_images_dir_2, dataset.query_path), transform=test_transformer),
        batch_size=48, num_workers=workers,
        shuffle=False, pin_memory=True)
    gallery_loader_2 = DataLoader(
        Preprocessor_occluded(dataset.gallery_2,
                     root=osp.join(dataset.target_images_dir_2, dataset.gallery_path), transform=test_transformer),
        batch_size=48, num_workers=workers,
        shuffle=False, pin_memory=True)
    # partial-REID
    query_loader_3 = DataLoader(
        Preprocessor_occluded(dataset.query_3,
                     root=osp.join(dataset.target_images_dir_3, dataset.query_path), transform=test_transformer),
        batch_size=48, num_workers=workers,
        shuffle=False, pin_memory=True)
    gallery_loader_3 = DataLoader(
        Preprocessor_occluded(dataset.gallery_3,
                     root=osp.join(dataset.target_images_dir_3, dataset.gallery_path), transform=test_transformer),
        batch_size=48, num_workers=workers,
        shuffle=False, pin_memory=True)
    return dataset, num_classes, source_train_loader, query_loader_2, gallery_loader_2, query_loader_3, gallery_loader_3
...
def main(args):
    ...
    if args.evaluate:
        print("Test:")
        print("partial-iLIDS:")
        evaluator.evaluate(query_loader_2, gallery_loader_2, dataset.query_2, dataset.gallery_2, args.output_feature)
        print("partial-REID:")
        evaluator.evaluate(query_loader_3, gallery_loader_3, dataset.query_3, dataset.gallery_3, args.output_feature)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch > 9 and epoch % 1 == 0:
            tmp_2=evaluator.evaluate(query_loader_2, gallery_loader_2, dataset.query_2, dataset.gallery_2, args.output_feature)
            tmp_3=evaluator.evaluate(query_loader_3, gallery_loader_3, dataset.query_3, dataset.gallery_3, args.output_feature)
        ...
        if(tmp_2>best_2):
            ...
            'TaskNet': TaskNet.module.state_dict(),
            'epoch': epoch + 1,
            }, fpath=osp.join(args.logs_dir, 'best_checkpoint_piLIDS.pth.tar'))
            best_2=tmp_2
